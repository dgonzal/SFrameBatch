...
if __name__ == "__main__":
    ...
    parser.add_option("-w", "--workdir",
                      ...
                      dest="workdir",
                      default="workdir",
                      help=" Place where to store overhead")
    ...
    if header.Version[0] == "-1":
        ...
        for cycle in Job.Job_Cylce:
            for process in range(len(cycle.Cycle_InputData)):
                ...
                NFiles.append(write_all_xml(workdir+'/'+header.Version[0],header,Job,workdir))
                write_script(header.Version[0],workdir)
                if(options.submit):submitt_qsub(NFiles[len(NFiles)-1],workdir+'/Stream_'+str(header.Version[0]),str(header.Version[0]),workdir)
            ...
            while loop_check==True:   
                ...
                for name in names:
                    ...
                    for it in range(NFiles[i]):
                        #print cycle.OutputDirectory+'# TODO: est/'+cycle.Cyclename+'.'+data_type[i]+'.'+names[i]+'_'+str(it)+'.root'
                        nameOfCycle = cycle.Cyclename.replace('::','.')
                        #print cycle.OutputDirectory+'/'+workdir+'/'+cycle.Cyclename+'.'+data_type[i]+'.'+names[i]+'_'+str(it)+'.root'
